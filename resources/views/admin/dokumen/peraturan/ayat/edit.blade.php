...
@section('content')
    <div class="card card-primary">
        ...
        <!-- /.card-header -->
        <!-- form start -->
        <form method="POST" action="{{ route('ayat.update', $ayat->id) }}" id="create-form">
            @csrf
            @method('put')
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <select class="select2" data-placeholder="Pilih Peraturan" style="width: 100%;" name="peraturan">
                        ...
                        @foreach ($peraturan as $item)
                            <option value="{{ $item->id }}" {{ $ayat->peraturan->id == $item->id ? 'selected' : '' }}>
                                {{ $item->nama }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="ayat" class="form-control @error('ayat') is-invalid @enderror"
                        id="ayat" placeholder="Masukkan Ayat" value="{{ $ayat->ayat }}">
                    @if ($errors->has('ayat'))
                ...
                <div class="form-group">
                    ...
                    <textarea name="isi" cols="30" rows="10" class="form-control @error('isi') is-invalid @enderror"
                        id="isi" placeholder="Masukkan isi">{{ $ayat->isi }}</textarea>
                    @if ($errors->has('isi'))
                ...
                <!-- /.form-group -->
                <div>
                    <a href="{{ route('pasal.show', $ayat->pasal->id) }}" class="btn btn-outline-secondary">Kembali</a>
                    <x-button-submit text="Edit Ayat" formId="create-form" />
                </div>
            </div>
        </form>
    </div>
...
@section('script')
    @php
        $selectedAyat = $ayat->pasal ? $ayat->pasal->id : null;
    @endphp
    <!-- Select2 -->
    ...
    <script>
        ...
        $('select[name="peraturan"]').on('change', function() {
            ...
            var selectedAyat = @json($selectedAyat);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data, function(key, value) {
                            $('select[name="pasal"]').append(
                                $('<option>', {
                                    value: key,
                                    text: value,
                                    selected: key === selectedAyat
                                })
                            );
                        });
