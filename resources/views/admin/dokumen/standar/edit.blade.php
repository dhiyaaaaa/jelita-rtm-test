...
@section('content')
    <div class="card card-primary">
        ...
        <!-- /.card-header -->
        <!-- form start -->
        <form method="POST" action="{{ route('standar.update', $standar->id) }}" id="create-form">
            @csrf
            @method('put')
            <div class="card-body">
                {{-- standar Input --}}
                <div class="form-group">
                    ...
                    <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror"
                        id="nama" placeholder="Masukkan Standar" value="{{ $standar->nama }}">
                    @if ($errors->has('nama'))
                ...
                <div class="form-group">
                    ...
                    <select class="select2" data-placeholder="Pilih Peraturan" style="width: 100%;" name="peraturan">
                        ...
                        @foreach ($peraturan as $item)
                            <option value="{{ $item->id }}"
                                {{ $standar->peraturan->id === $item->id ? 'selected' : '' }}>{{ $item->nama }}
                            </option>
                        @endforeach
                    </select>
                 ...
                 <div class="form-group">
                    ...
                    <input type="text" name="kode" class="form-control @error('kode') is-invalid @enderror"
                        id="kode" placeholder="Masukkan Kode" value="{{ $standar->kode }}">
                    @if ($errors->has('kode'))
                ...
                <div>
                    ...
                    <x-button-submit text="Edit Standar" formId="create-form" />
                </div>
            </div>
        </form>
