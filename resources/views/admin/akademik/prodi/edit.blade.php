...
@section('content')
    <div class="card card-primary">
        ...
        <!-- /.card-header -->
        <!-- form start -->
        <form method="POST" action="{{ route('prodi.update', $prodi->id) }}" id="edit-form">
            @csrf
            @method('put')
            <div class="card-body">
                {{-- Prodi Input --}}
                <div class="form-group">
                    ...
                    <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror"
                        id="nama" placeholder="Masukkan Nama Prodi" value="{{ $prodi->nama }}">
                    @if ($errors->has('nama'))
                ...
                <div class="form-group">
                    ...
                    <select class="select2" data-placeholder="Pilih Fakultas" style="width: 100%;" name="fakultas">
                        ...
                        @foreach ($fakultas as $fak)
                            <option value="{{ $fak->id }}"
                                {{ $prodi->fakultas->id == $fak->id ? 'selected' : '' }}>{{ $fak->nama }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="select2" data-placeholder="Pilih Jenjang" style="width: 100%;" name="jenjang">
                        ...
                        @foreach ($jenjang as $item)
                            <option value="{{ $item->id }}" {{ $prodi->jenjang->id == $item->id ? 'selected' : '' }}>{{ $item->nama }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-button-submit text="Edit Prodi" formId="edit-form" />
                </div>
            </div>
        </form>
