...
@section('content')
    <div class="card card-primary">
        ...
        <!-- form start -->
        <form method="POST"
            action="{{ route('auditee.update', ['jadwalAudit' => $jadwalAudit->id, 'unit' => $unit, 'type' => $type]) }}"
            id="edit-form">
            @csrf
            @method('put')
            <div class="card-body">
                <div class="form-group">
                    ...
                    <select class="select2" multiple data-placeholder="Pilih Auditan" style="width: 100%;" name="auditee[]">
                        @foreach ($user as $us)
                            <option value="{{ $us->id }}"
                                {{ in_array($us->id, $auditee->toArray()) ? 'selected' : '' }}>
                                {{ $us->name . ' - ' . $us->jabatan[0]->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-button-submit text="Edit Auditan" formId="edit-form" />
                </div>
            </div>
        </form>
