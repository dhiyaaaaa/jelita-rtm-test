...
class HasilRtmController extends Controller
{
    protected $user;
    ...
    {
        $fakultasId = $this->user->jabatan->pluck('pivot.fakultas_id')->filter()->first();
        $unitId = $this->user->jabatan->pluck('pivot.unit_id')->filter()->first();

        $jadwal = JadwalAudit::with([
            'rtm_rtl',
            'rtm_rtl_univ' => function($query) use ($fakultasId, $unitId) {
                $query->where(function($q) use ($fakultasId, $unitId) {
                    if ($fakultasId) {
        ...
        return view('tindak_lanjut.rtm_fakultas.hasil_rtm.index', $data);
    }
