...
class RtmRtlProdiController extends Controller
{
    ...
    public function save_form(Request $request, string $rtmRtl, string $auditee): JsonResponse
    {
        ...
        try {
            ...
            foreach ($kriteriaList as $kriteria) {
                ...
                // Hapus data lama yang tidak ada dalam tindakanList
                RtmTindakLanjut::where('rtm_rtl_id', $rtmRtl)
                    ->where('form_id', $formId)
                    ->where('kriteria_id', $kriteriaId)
                    ->whereNotIn('id', collect($tindakanList)->pluck('id')->filter())
                    ->delete();
    ...
    public function store_form(Request $request, RtmRtl $rtmRtl, string $auditee): RedirectResponse
    {
        ...
        foreach ($sessionFormData as $page => $data) {
            foreach ($data as $key => $value) {
                ...
                if (preg_match('/^(tindakan_|pic_|waktu_)(\d+)$/', $key, $matches)) {
                    ...
                    // Validasi setiap input yang dibutuhkan
        ...
        // Menyimpan atau memperbarui tindak lanjut dari setiap form
        foreach ($requestData as $key => $value) {
            if (preg_match('/^(tindakan_|pic_|waktu_)(\d+)$/', $key, $matches)) {
                ...
                // Ambil Kriteria berdasarkan form_id
                ...
                if ($kriteria) {
                    ...
                    RtmTindakLanjut::updateOrCreate(
                        ...
                        [
                            'auditee_id' => $auditee,
                            'kriteria_id' => $kriteria->id,  // Menggunakan kriteria_id yang diambil
                            'pic' => $pic,
                            'waktu' => $waktu,
                        ]
                    );
    ...
    public function updateStatus(Request $request, RtmRtl $rtmRtl): JsonResponse
    {
        $status = StatusRtmRtl::where('rtm_rtl_id', $rtmRtl->id)->first();
        ...
        if ($status && $status->status == 'in_progress') {
            ...
            Log::info('Status updated by RtmRtlProdiController:', [
                'rtm_rtl_id' => $rtmRtl->id,
                'status' => $status->status,
            ]);

            return response()->json([
                'message' => 'Status berhasil diubah menjadi completed.',
                'redirect' => route('dekan.jadwal-rtm.index') // Redirect ke halaman index
            ], 200);
        } else {
            return response()->json(['message' => 'Status tidak ditemukan atau sudah completed.'], 400);
        }
    }
}
